...
def _sre_sigterm_handler(signum, frame):
    ...
    _logger.warning(f"🚨 [SRE] Sinal {signum} recebido. Propagando SIGTERM para {len(_CHILD_PROCESSES)} processo(s) filho(s)...")
...
async def run_training_script(name: str, entrypoint: Path, project_path: Path, env: dict):
    logger = logging.getLogger(__name__)
    logger.info(f"[{name}] Iniciando {entrypoint.name}...")
    process = None
    try:
        process = await asyncio.create_subprocess_exec(
            VENV_PYTHON, str(entrypoint),
...
def task_build_information_bars(asset: str, base_timeframe: str = "M5") -> str:
    ...
    for candidate_asset in possible_assets:
        for dtype in ["futures", "spot"]:
            candidate_dir = ProjectPaths.PROJECT_ROOT / "data" / "gold_delta" / f"data_type={dtype}" / f"symbol={candidate_asset}" / f"timeframe={base_timeframe}"
    ...
    if not gold_dir or not gold_dir.exists():
        logger.error(f"❌ Dados base {base_timeframe} ausentes para {asset}. A DAG falhará (Tolerância Zero).")
        raise ValueError(f"Dados {base_timeframe} ausentes para o derivativo {asset}.")
            
    files = list(gold_dir.rglob("*.parquet"))
    if not files:
        raise ValueError(f"Nenhum arquivo Parquet encontrado para {asset} em {gold_dir}")
    ...
    if 'close' not in df.columns:
        ...
        if c_col: df = df.rename({c_col[0]: 'close'})
    ...
    if 'volume' not in df.columns:
        ...
        if v_col: 
            ...
            raise ValueError(f"❌ Coluna volume ausente em {asset}. Fallback sintético proibido pelas regras SRE.")
    ...
    out_tf = "SOTA_VOL"
    out_dir = ProjectPaths.PROJECT_ROOT / "data" / "gold_delta" / "data_type=futures" / f"symbol={asset}" / f"timeframe={out_tf}"
    out_dir.mkdir(parents=True, exist_ok=True)
    
    sota_df.write_parquet(out_dir / "data.parquet")
    logger.info(f"✅ Filtro Entrópico SOTA concluído para o derivativo {asset}. Timeframe Alvo: {out_tf}")
    return out_tf
...
async def run_ml2_hft(dry_run: bool, asset: str, timeframe: str):
    ...
    logger = logging.getLogger(__name__)
    logger.info(f"🤖 [ML-2] Iniciando HFT PPO Training para {asset} no timeframe {timeframe}...")
...
async def task_run_coint_futures(dry_run: bool, symbols: list):
    ...
    """
    logger = logging.getLogger(__name__)
    logger.info(f"🔗 [ML-0] Analisando Cointegração e Spread de Futuros para: {symbols}...")
...
def task_run_hawkes_futures(asset: str, timeframe: str = "M5"):
    ...
    """
    from app.training.ml_4_series_temporais.hawkes_microstructure import MultivariateHawkesEstimator
    from app.training.shared_lib.paths import ProjectPaths
    import polars as pl
    import numpy as np
    logger = logging.getLogger(__name__)
    logger.info(f"⚡ [ML-4 Hawkes] Calculando intensidade de fluxo de ordens e Branching Ratio para {asset}...")
    
    gold_dir = ProjectPaths.get_gold_path(data_type="futures", asset=asset, timeframe=timeframe)
    if not gold_dir.exists() or not list(gold_dir.rglob("*.parquet")):
        gold_dir = ProjectPaths.get_silver_path(data_type="futures", asset=asset, timeframe=timeframe)
    ...
    if gold_dir.exists() and list(gold_dir.rglob("*.parquet")):
        ...
        if "close" in df.columns and len(df) > 5:
            ...
            logger.info(f"📊 [ML-4 Hawkes {asset}] Branching Ratio: {res.branching_ratio:.4f} | Cascade: {res.is_supercritical_cascade} | TSRV Vol: {res.tsrv_volatility:.6f}")
...
def task_run_feature_selection_futures(asset: str, timeframe: str = "M5"):
    ...
    from app.training.shared_lib.paths import ProjectPaths
    logger = logging.getLogger(__name__)
    logger.info(f"🎯 [3-Stage Feature Selection] Executando Clustered Boruta SHAP para {asset}...")

    gold_df = PolarsDeltaRepository.load_data(layer="gold", data_type="futures", symbol=asset, timeframe=timeframe, base_path=str(ProjectPaths.DATA_DIR))
    if gold_df.is_empty():
        gold_df = PolarsDeltaRepository.load_data(layer="silver", data_type="futures", symbol=asset, timeframe=timeframe, base_path=str(ProjectPaths.DATA_DIR))

    if not gold_df.is_empty():
        df_pd = gold_df.to_pandas()
        candidate_cols = [c for c in df_pd.columns if c not in ["time", "symbol", "data_type", "timeframe", "open", "high", "low", "close"]]
        if len(candidate_cols) > 1:
            pipeline = FeatureSelectionPipeline(n_estimators=30, max_iter=5, random_state=42)
            accepted, summary = pipeline.run_selection(df_pd, feature_cols=candidate_cols, target_col="log_return")
            logger.info(f"✅ [3-Stage Feature Selection {asset}] {len(accepted)}/{len(candidate_cols)} features aceitas: {accepted}")
            ...
            with open(meta_file, "w") as f:
                json.dump({"asset": asset, "accepted_features": accepted, "summary": summary}, f, indent=2)
...
def task_run_hft_backtest(asset: str, timeframe: str = "M5"):
    ...
    if report:
        ...
        logger.info(f"📈 [HFT Backtest {asset}] Sharpe: {metrics.get('sharpe_ratio')} | Sortino: {metrics.get('sortino_ratio')} | Max DD: {metrics.get('max_drawdown_pct')}% | Win Rate: {metrics.get('win_rate_pct')}%")

@flow(name="HFT Sniper Master DAG (Futures Derivatives)")
async def futures_master_dag(dry_run: bool = False, symbols: list = None, base_timeframe: str = "M5"):
    ...
    logger.info("🛡️ SRE Constraint: Pulando seleção de portfólio de Markowitz (ML-5). Futuros treinam HFT direto.")
    
    # 0. Ingestão Automática no Lakehouse (3500 barras para convergência estatística do GMM)
    logger.info(f"📥 Ingerindo dados base {base_timeframe} para derivativos no Lakehouse...")
    await asyncio.to_thread(call_command, 'run_lakehouse_pipeline', symbols=symbols, timeframe=base_timeframe, bars=50000, data_type='futures')
    ...
    for asset in symbols:
        ...
        try:
            ...
            await asyncio.to_thread(task_run_feature_selection_futures, asset, base_timeframe)

            # 3. Filtro Entrópico: Converte M5/M1 em Volume Bars purificadas via GMM
            sota_tf = await asyncio.to_thread(task_build_information_bars, asset, base_timeframe)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symbols", type=str, default="WIN$,WDO$", help="Futuros separados por vírgula")
    parser.add_argument("--base-tf", type=str, default="M5", help="Timeframe de base para o HFT (M1 ou M5)")
    args = parser.parse_known_args()[0]
    
    symbols_list = [s.strip() for s in args.symbols.split(',') if s.strip()]
    asyncio.run(futures_master_dag(dry_run=args.dry_run, symbols=symbols_list, base_timeframe=args.base_tf))
