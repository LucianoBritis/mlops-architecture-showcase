...
class SilverCleanser:
    ...
    def clean_candles(self, raw_data: Union[List[Dict[str, Any]], pd.DataFrame, pl.DataFrame, pl.LazyFrame], symbol: str) -> pl.LazyFrame:
        if isinstance(raw_data, list) and not raw_data:
            return pl.LazyFrame()
        elif isinstance(raw_data, pd.DataFrame) and raw_data.empty:
            return pl.LazyFrame()
        ...
        try:
            ...
            time_col = "timestamp" if "timestamp" in lf.collect_schema().names() else "time"
            ...
            if lf.collect_schema()[time_col] in [pl.Datetime, pl.Date]:
                ...
                time_expr = pl.coalesce([
                    pl.col(time_col).str.strptime(pl.Datetime, "%a, %d %b %Y %H:%M:%S GMT", strict=False),
                    pl.col(time_col).str.strptime(pl.Datetime, "%a, %d %b %Y %H:%M:%S %Z", strict=False),
            ...
            lf = (
                lf.unique(subset=[time_col], keep="last")
                  ...
                  .with_columns([
                      pl.when((pl.col("time").dt.hour() == 0) & (pl.col("time").dt.minute() == 0))
            ...
            required_cols = ["symbol", "time", "open", "high", "low", "close", "tick_volume", "spread", "real_volume"]
            ...
            if "original_contract" in lf.collect_schema().names() and ("WIN$" in symbol or "WDO$" in symbol):
                lf = lf.sort("time")
                
                # Detecta as fronteiras de rolagem (is_rollover = True quando muda o contrato)
                lf = lf.with_columns(
                    (pl.col("original_contract") != pl.col("original_contract").shift(1)).fill_null(False).alias("is_rollover")
                )
                ...
                lf = lf.with_columns(
                    pl.col("rollover_gap").shift(-1).fill_null(0).reverse().cum_sum().reverse().alias("cum_gap")
                )
                
                # Aplica o gap acumulado a todas as colunas de preço
                lf = lf.with_columns([
                    pl.col("open") + pl.col("cum_gap"),
                    pl.col("high") + pl.col("cum_gap"),
                    pl.col("low") + pl.col("cum_gap"),
                    pl.col("close") + pl.col("cum_gap")
                ])
                
                # Dropa as colunas temporárias
                lf = lf.drop(["is_rollover", "rollover_gap", "cum_gap", "original_contract"])
            ...
            return lf.sort("time").drop_nulls(subset=["time", "open", "high", "low", "close"])
        except Exception:
            logger.exception("Erro durante o processamento da camada Silver para Candles")
            return pl.LazyFrame()

    def save_silver_candles(self, lf: pl.LazyFrame, symbol: str, timeframe: str, base_path: str = "data", data_type: str = None) -> None:
        ...
        try:
            ...
            df = lf.collect().drop_nulls(subset=["time", "open", "high", "low", "close"])
            df = df.with_columns([
                pl.col("tick_volume").fill_null(0),
                pl.col("real_volume").fill_null(0),
                pl.col("spread").fill_null(0)
            ...
            if df.is_empty():
                logger.info(f"ℹ️ [Silver Layer] Nenhum novo incremento de barras para {symbol} ({timeframe}). Base mantida íntegra.")
                return
            ...
            logger.info("Dados Silver salvos com sucesso (Garantia Zero Nulos) no Delta Lake %s (%s)", symbol, timeframe)
        except Exception:
            logger.exception("Erro ao salvar candles higienizados na camada Silver Delta")
            raise
