...
class LocalParquetBronzeIngestor(BronzeIngestor):
    ...
    def save_raw_ticks(self, symbol: str, ticks: list[dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if "time_msc" in last_tick:
                ...
                if drift > 0.010:  # 10ms
                    logger.critical(
                        f"🛑 [SRE CIRCUIT BREAKER] Clock Drift Crítico detectado ({drift:.4f}s > 10ms) para {symbol}. Lote descartado (Drop Tail) para evitar execução HFT envenenada."
                    )
                    return False
            ...
            logger.info(
                "Persistindo %d ticks brutos na Delta Table Bronze para %s",
                len(ticks),
                symbol,
            )
            return True
    ...
    def save_raw_candles(
        self, symbol: str, timeframe: str, raw_data: Any, data_type: str | None = None
    ) -> bool:
        try:
            ...
            logger.info(
                "Persistindo candles brutos na Delta Table Bronze para %s (%s)",
                symbol,
                timeframe,
            )
            return True
